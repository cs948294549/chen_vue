    ...
    <el-scrollbar class="log-scroll-container" height="100%">
      ...
      <div class="log-list">
        ...
        >
          ...
          <!-- 日志内容（等宽字体，还原服务器日志效果） -->
          <span class="handler-time">{{ log.formatCreateTime }}</span>
          <span class="handler-name">用户 {{ log.handler }}</span>
          <span class="handler-msg">{{ log.msg }}</span>
...
export default {
  ...
  computed: {
    ...
    formattedLogData() {
      return this.logData.map(log => ({
        ...
        // 格式化创建时间（提升可读性，处理空值/非法值）
        formatCreateTime: this.formatTime(log.create_time)
      }));
    }
  },
  mounted() {
  ...
  methods:{
    // 时间格式化工具方法（兼容时间戳、字符串格式，返回 YYYY-MM-DD HH:mm:ss）
    formatTime(time) {
      if (!time) return '未知时间';
      // 处理时间戳（秒级/毫秒级）
      time = time.toString()
      let date = new Date();
      date = new Date(time.length === 10 ? time * 1000 : time);
...
/* 处理内容样式（默认颜色，保持可读性） */
.handler-msg {
  color: #f8f9fa; /* 亮白色，适配深色背景 */
  margin-right: 8px;
}

/* 处理时间样式（弱化展示，区分内容） */
.handler-time {
  color: #90a4ae; /* 浅灰蓝色，弱化时间展示，不抢焦点 */
  font-size: 13px; /* 轻微缩小字体，区分主次 */
}
