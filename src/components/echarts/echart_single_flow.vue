<template>
	<div>
	  <div style="text-align: center;font-weight: bolder;">流量计数</div>
	  ...
	  methods: {
			...
			getDevFlow:function(){
				...
				let in_p = new Promise(function(resolve,reject){
				  collector_api.getSNMPGET({"ip":ip, "oid":"1.3.6.1.2.1.31.1.1.1.6."+port_id, "coding":"utf-8"},{}).then(function (response) {
				...
				let out_p = new Promise(function(resolve,reject){
				  collector_api.getSNMPGET({"ip":ip, "oid":"1.3.6.1.2.1.31.1.1.1.10."+port_id, "coding":"utf-8"},{}).then(function (response) {
				    // console.log("===", response)
				...
				Promise.all([in_p, out_p]).then(function(res){
				  ...
				  for(let i=0;i<res.length;i++){
				    if(res[i]["name"]=="in"){
				      if(that.flow_in_curr===""){
				        // console.log("修改in====", that.flow_in_curr)
				    ...
				    if(res[i]["name"]=="out"){
				      if(that.flow_out_curr===""){
				        // console.log("修改out====", that.flow_out_curr)
				  ...
				  // console.log(flow_info, that.flow_in_curr, that.flow_out_curr, "---")
        ...
        // console.log("添加flow数据====", data)
				...
				let hex="b";
				let division=1;
				let max=Math.max(out_sp,in_sp);
				if(parseInt(max/1000)>0){
					hex="K";
				...
				if(this.division_flag<division){
					for(let i=0;i<this.flow_in.length-1;i++){
						this.flow_in[i]=parseFloat(this.flow_in[i]*this.division_flag/division).toFixed(2);
						this.flow_out[i]=parseFloat(this.flow_out[i]*this.division_flag/division).toFixed(2);
					}
					this.flow_in[this.flow_in.length-1]=parseFloat(this.flow_in[this.flow_in.length-1]/division).toFixed(2);
					this.flow_out[this.flow_out.length-1]=parseFloat(this.flow_out[this.flow_in.length-1]/division).toFixed(2);

					this.division_flag=division;
					this.hex_flag=hex;
					// console.log("更换进制>",division,hex,this.division_flag,this.hex_flag);
				}else if(this.division_flag>division){
					this.flow_in[this.flow_in.length-1]=parseFloat(this.flow_in[this.flow_in.length-1]/this.division_flag).toFixed(2);
					this.flow_out[this.flow_out.length-1]=parseFloat(this.flow_out[this.flow_in.length-1]/this.division_flag).toFixed(2);
					hex=this.hex_flag;
					// console.log("更换进制<",division,hex,this.division_flag,this.hex_flag);
				}else{
					// console.log("保持==",division,hex,this.division_flag,this.hex_flag);
					this.flow_in[this.flow_in.length-1]=parseFloat(this.flow_in[this.flow_in.length-1]/this.division_flag).toFixed(2);
					this.flow_out[this.flow_out.length-1]=parseFloat(this.flow_out[this.flow_in.length-1]/this.division_flag).toFixed(2);
				}

        // console.log("数据========", this.flow_in, this.flow_out)
